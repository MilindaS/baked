...
require_once dirname(__FILE__) .'/../modules/Classes/PHP_XLSXWriter/xlsxwriter.class.php';
...
class Cli extends CI_Controller
{
    ...
    static $asy_pquery = array();
    static $asy_wquery = array();
...
";
    ...
    public static function doGenerateQueryReport($job){
        ...
        array_push(Cli::$asy_pquery, $data['asyp_query']);
        array_push(Cli::$asy_wquery,  $data['asyw_query']);
        ...
        try{
          ...
          $id = Cli::$rec_id;
          Cli::cancelGeneration($e->getMessage(),2,$id);
        }
    }
    ...
    public static function generateExcel($stidArr_im=null,$stidArr_ex=null,$stidArr_grn=null,$detail=null){
        ...
        if($detail != null && (isset($detail['sheet']) && !empty($detail['sheet']))){
          ...
          $writer->writeSheetHeader('Detail', $header );//optional

          foreach($detail['sheet'] as $stid){
            while ($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)) {
              if(!empty($row) && count($row)>0){
                   $writer->writeSheetRow('Detail',$row);
              }
            }
          }
      }
      ...
      if($stidArr_im!=null ){
        ...
        $writer = new XLSXWriter();
        $writer->writeSheetHeader('Import', $header );//optional

        foreach($stidArr_im as $stid){
          while ($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)) {
            if(!empty($row) && count($row)>0){
                 $writer->writeSheetRow('Import',$row);
            }
          }
        }
      }
      ...
      if($stidArr_grn!=null ){
        ...
        $writer->writeSheetHeader('GRN', $header );//optional

        foreach($stidArr_grn as $stid){
          while ($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)) {
            if(!empty($row) && count($row)>0){
                 $writer->writeSheetRow('GRN',$row);
            }
          }
        }
      }
        ...
        $writer->writeToFile(__DIR__.'/../../public/uploads/genr/'.$report_name);
        if(!file_exists(__DIR__.'/../../public/uploads/genr/'.$report_name)){
    ...
    public static function storegf($filename,$file,$execution_time,$id){
        $status = 1;
        $sql_z = json_encode(Cli::$asy_pquery).json_encode(Cli::$asy_wquery);
        $sql = "UPDATE generatedFiles set filename = ?, file = ?, execution_time = ?, sql_query =?, status = ? WHERE id =?";
        $params = array($filename,$file,$execution_time,$sql_z,$status,$id);
        $CI =& get_instance();
        $query = $CI->db->query($sql,$params);
        return true;
    }

    public static function cancelGeneration($messege,$status,$id){

        $sql = "UPDATE generatedFiles set messege = ?, status = ? WHERE id =?";
        $params = array($messege,$status,$id);
        $CI =& get_instance();
        $query = $CI->db->query($sql,$params);
        return true;
    }
